...
const COLORS = ['#0d9488', '#f59e0b', '#e11d48', '#6366f1', '#c2410c'];
  ...
  return (
    <main className="flex-1 w-full max-w-[1440px] mx-auto p-4 md:p-8 lg:px-10 flex flex-col gap-8 bg-slate-50">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { title: "Consumo Semanal", val: stats.weeklyUnits, icon: CheckCircle, trend: stats.prevWeeklyUnits > 0 ? `${((stats.weeklyUnits - stats.prevWeeklyUnits) / stats.prevWeeklyUnits * 100).toFixed(0)}%` : '-', trendIcon: TrendingUp, trendCol: stats.weeklyUnits > stats.prevWeeklyUnits ? "text-rose-500" : "text-emerald-500", footer: "vs sem. ant." },
          { title: "Día Mayor Consumo", val: stats.highestDay.units, icon: Flame, footer: stats.highestDay.date },
          { title: "Promedio Diario", val: stats.dailyAverage.toFixed(1), icon: TrendingUp, footer: "esta semana" },
          { title: "Ahorro Estimado", val: `$${stats.savings}`, icon: PiggyBank, footer: "por elegir sin alcohol" }
        ].map((card, idx) => {
          ...
          return (
            <div key={idx} className="bg-white rounded-xl p-6 border border-slate-200 shadow-sm flex flex-col justify-between relative overflow-hidden group">
              <div className="absolute right-0 top-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                <Icon size={80} className="text-teal-600" />
              </div>
          ...
          <div className="flex-1 min-h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={stats.barChartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                />
                <Bar dataKey="unidades" fill="#0d9488" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
